...
export function MediumImpactHero({ block }: Props) {
  return (
    <section className="bg-[var(--color-bg-alt,#f8fafc)] overflow-hidden">
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-16 px-6 py-20 md:grid-cols-[55fr_45fr] md:py-28 lg:px-12">
        <motion.div {...fadeInUp}>
          {block.badge && (
          ...
          {block.subheading && (
            <p className="text-lg text-[var(--color-text-muted,#64748b)] leading-relaxed mb-8 max-w-lg">
              {block.subheading}
            </p>
          )}
          {block.ctaLabel && block.ctaLink && (
            <a
              href={block.ctaLink}
              className="group inline-flex items-center gap-2.5 rounded-[var(--radius,0.5rem)] bg-[var(--color-primary,#0f172a)] px-8 py-4 text-base font-semibold text-white shadow-depth transition-shadow transition-transform duration-300 hover:shadow-depth-lg hover:-translate-y-0.5"
            >
              {block.ctaLabel}
